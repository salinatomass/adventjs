// Reto: https://adventjs.dev/challenges/19
...
function learn(time, courses) {
  ...
  for (let i = 0; i < courses.length; i++)
    for (let j = i + 1; j < courses.length; j++)
      hours.push({ courseA: i, courseB: j, total: courses[i] + courses[j] });

  const result = hours
    .sort((a, b) => b.total - a.total)
    .find(hour => hour.total <= time);

  return result ? [result.courseA, result.courseB] : null;
}
